...
function sCreCal($noDate, $monDays, $iYear, $iMon, $iDate){
	...
	for ($i = 1; $i <= $cell; $i++){
		...
		// *拿年月以及第幾個星期幾去問是不是假日
		$specH = specHoliday_tw($iMon, $noDate);
		if ($specH[$iMon][0] == $iDays && $iDays !== 0){
			$str [$i][2]=' class="holiday"';
			$str [$i][4][0]=$iDays;
			array_push($str [$i][4][3], $specH[$iMon][2]);
			$str [$i][4][-1]='';
			$str [$i][4][5]='';
		}
		// *拿年月日去問是不是假期
	...
	$sCreCal = implode("",$str); // 把一維陣列組成字串傳出function讓外面使用
	return $sCreCal;
}
// *拿年月日去問是不是假日
/*
...
*/
function specHoliday_tw($iMon, $noDate){
	// *做出年月以及第幾周的第幾個星期幾的對照表
	// 日期的格子轉換成第幾個星期幾
	// $specH [月][第幾個][星期幾] = array(是否為節日, 節慶名稱, 第幾個星期幾是幾號);---組合不出來？---
	// 星期日 = 1，星期一 = 2...
	$specH = array();
	$a [2][1] = ($noDate==0)?8:(15-$noDate);
	// 五月的第二個禮拜日
	$specH [5] = array($a[2][1], 1, '母親節');
	// 十一月的第四個星期四
	// $specH [11] = array(4, 4, 1, '感恩節', ($noDate >= 5)?(33-$noDate):(26-$noDate));
	return $specH;
}
// *拿年月日去問是不是假期
/*

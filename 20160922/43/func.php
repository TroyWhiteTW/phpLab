...
function sCreCal($noDate, $monDays, $iYear, $iMon, $iDate){
	...
	$k = holiday_1($iMon); // 放假的日期
	$j = $k+$noDate; // 放假的日期
	// 判斷假日的前一天或後一天是不是周末，再判斷前二天或後二天，
	if ($str [$j+1][2]==' class="weekend"'){
		if ($str [$j+2][2]==' class="weekend"'){
			$str [$j+2][2]=' class="cHoliday"';
			$str [$j+1][2]=' class="cHoliday"';
			$str [$j][2]=' class="cHoliday"';
		}
	}
	if ($str [$j-1][2]==' class="weekend"'){
		if ($str [$j-2][2]==' class="weekend"'){

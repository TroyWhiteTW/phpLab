<?php 
	...
	function read_entries ( $m, $y, $d, $logged_in, $start_entry, $category ) {
			...
			for ( $i = 0; $i <= count( $contents ) - 1; $i++ ) {
				...
				*/
				...
				if ( $blog_config[ 'blog_enable_comments' ] == true ) {
					...
					if ( $view_counter > 0 ) {
						if ( $view_counter == 1) {
							$entry_array[ 'comment' ][ 'count' ] = $lang_string[ 'sb_view_counter_pre' ] ;
							$entry_array[ 'comment' ][ 'count' ].= $view_counter . $lang_string[ 'sb_view_counter_post' ];
						} else {
							$entry_array[ 'comment' ][ 'count' ] = $lang_string[ 'sb_view_counter_plural_pre' ] ;
							$entry_array[ 'comment' ][ 'count' ].= $view_counter . $lang_string[ 'sb_view_counter_plural_post' ];
						}
					}
				}
		...
		if ( $previous_entry != NULL ) {$blog_content.= show_previous_entry_tag($next_entry,$category);}

		if ( $next_entry != NULL ) {$blog_content.= show_next_entry_tag($next_entry,$category);}
...
//==========================================================================
	function show_previous_entry_tag($previous_entry,$category){
		global $lang_string;
		$previous_entry_tag='';
		list( $entry_filename, $year_dir, $month_dir ) = explode( '|', $previous_entry );
		$d = substr( $entry_filename, 9, 2 );
		$previous_entry_tag.=' <span style="float: left;">';
		$previous_entry_tag.='<a href="index.php?m=' . $month_dir ;
		$previous_entry_tag.= '&amp;y=' . $year_dir ;
		$previous_entry_tag.= '&amp;d=' . $d ;
		$previous_entry_tag.= '&amp;entry=' . sb_strip_extension( $entry_filename );
		if ( $category != NULL ) {
			$previous_entry_tag.= '&amp;category=' . $category;
		}
		$previous_entry_tag.= '">' . $lang_string[ 'nav_back' ] . '</a></span> ';
		return $previous_entry_tag;
	}
	function show_next_entry_tag($next_entry,$category){
		global $lang_string;
		$next_entry_tag='';
		list( $entry_filename, $year_dir, $month_dir ) = explode( '|', $next_entry );
		$d = substr( $entry_filename, 9, 2 );
		$next_entry_tag.=' <span style="float: right;">';
		$next_entry_tag.= ' <a href="index.php?m=' . $month_dir;
		$next_entry_tag.= '&amp;y=' . $year_dir . '&amp;d=' . $d ;
		$next_entry_tag.= '&amp;entry=' . sb_strip_extension( $entry_filename );
		if ( $category != NULL ) {
			$next_entry_tag.='&amp;category=' . $category;
		}
		$next_entry_tag.='">' . $lang_string[ 'nav_next' ] . '</a></span> ';
		return $next_entry_tag;
	}
...
function get_entry_title ( $y, $m, $entry ) {	
	...
	function read_entry_from_file ( $entry_id ) {
		...
		// Categories
		if ( array_key_exists( 'CATEGORIES', $blog_entry_data ) ) {
			$temp_cat_array =(!stristr(",",$blog_entry_data[ 'CATEGORIES' ]))?array($blog_entry_data[ 'CATEGORIES' ]): explode( ',', $blog_entry_data[ 'CATEGORIES' ] );
			$temp_cat_names = Array();
			for ( $j = 0; $j < count( $temp_cat_array ); $j++ ) {
				array_push( $temp_cat_names, get_category_by_id ( $temp_cat_array[$j] ) );
			}
